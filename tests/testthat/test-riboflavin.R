test_that("riboflavin dataset has the expected structure", {
  ...
  expect_equal(dim(riboflavin$A), c(71, 932))
  ...
  expect_length(riboflavin$group, 932)
  expect_length(unique(riboflavin$group), 94)
  expect_true("bsu00740" %in% riboflavin$group) # Riboflavin metabolism itself
})
...
test_that("riboflavin fits cleanly through the public API", {
  ...
  expect_equal(dim(fit$x), c(932, 1))
  ...
  expect_equal(dim(fit_auto$x), c(932, 5))
})
